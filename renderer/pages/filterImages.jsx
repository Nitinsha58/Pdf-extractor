...
export default function FilterImagesPage() {
	...
	const [meta, setMeta] = useState({ page: 1, page_size: 60, count: 0 });
	...
	const groupedItems = useMemo(() => {
		...
		return (Array.isArray(items) ? items : []).map((it, idx) => {
			...
			const primaryUrl = buildMediaUrl(it?.cropped_image || it?.image);
			...
			if (primaryUrl) {
				images.push({
					key: `primary-${it?.id ?? idx}`,
					url: primaryUrl,
					label: String(groupNumber),
				});
			}
			extras.forEach((ex, exIdx) => {
				...
				if (!url) return;
				images.push({
					key: `extra-${it?.id ?? idx}-${ex?.id ?? exIdx}`,
					url,
					label: `${groupNumber}.${exIdx + 1}`,
				});
			});
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<div className="text-sm text-gray-600">
								Filter and copy images (no edit form)
							</div>
						</div>
					</div>
								...
								<div
									...
									<div className="flex flex-col gap-3">
										{g.images.map((img) => (
											<div
												key={img.key}
												className="relative w-full border border-gray-200 bg-white overflow-hidden">
