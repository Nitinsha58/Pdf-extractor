...
export default function Classes() {
	...
	useEffect(() => {
		...
		const el = document.querySelector(
			`input[data-class-name-id="${CSS.escape(String(lastAddedId))}"]`
		);
	...
	const saveChanges = async () => {
		...
		setIsSaving(true);
		try {
			const resp = await fetch(`${API_BASE}/api/classes/bulk/`, {
				method: "POST",
			...
			// Reload from server to get real IDs for created rows
			const listResp = await fetch(`${API_BASE}/api/classes/`);
				...
				<div className="bg-white shadow rounded-lg p-4 mb-4">
					<div className="flex gap-3 items-center">
						<input
							...
							placeholder="New class name (press Enter)"
						/>
						<button
								...
								<tr key={item.id}>
									...
									<td className="px-6 py-4">
										<input
											...
											data-class-name-id={String(item.id)}
